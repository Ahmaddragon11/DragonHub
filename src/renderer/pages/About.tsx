        ...
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <section className="card p-5">
              ...
              <dl className="text-sm space-y-1.5">
                {v && Object.entries({ version: v.version, build: v.build, releaseDate: v.releaseDate, electron: v.electron, chrome: v.chrome, node: v.node, platform: `${v.platform} ${v.arch}` }).map(([k, val]) => (
                  <div key={k} className="flex justify-between gap-3 border-b border-surface-300/50 pb-1"><dt className="opacity-50 capitalize">{k}</dt><dd className="font-mono text-xs" dir="ltr">{val}</dd></div>
                ))}
              </dl>
            ...
            <section className="card p-5">
              ...
              <p className="text-sm opacity-70 leading-relaxed">{t('about.licenseText')}</p>
              <div className="mt-4 p-3 rounded-xl bg-surface-200 text-xs font-mono" dir="ltr">{t('about.developer')}: <b>{DEVELOPER}</b><br />{t('about.contact')}: {TELEGRAM_URL}<br />DragonHub v{APP_VERSION} (Windows 10/11 x64)</div>
            </section>
          </div>
