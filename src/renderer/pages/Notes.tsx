  ...
  return (
    <div className="page-enter h-full flex flex-col">
      <PageHeader icon={<StickyNote size={22} />} title={t('notes.title')} subtitle={t('common.items', { count: notes.length })}>
        <select className="select w-auto text-xs" value={sort} onChange={(e) => setSort(e.target.value as any)}>
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-4">
        <aside className="card flex flex-col min-h-0 overflow-hidden">
          <div className="p-3 space-y-2 border-b border-surface-300/60">
            ...
            {allTags.length > 0 && <div className="flex gap-1 flex-wrap">{allTags.map((tg) => <button key={tg} onClick={() => setTagFilter(tagFilter === tg ? null : tg)} className={cn('badge cursor-pointer', tagFilter === tg ? 'bg-accent text-accent-fg' : 'bg-surface-200 text-surface-700')}><Tag size={10} />{tg}</button>)}</div>}
          </div>
          <div className="flex-1 overflow-auto p-2 space-y-1 stagger">
            {list.length === 0 && <p className="text-xs text-surface-500 text-center p-6">{t('common.noResults')}</p>}
            {list.map((n) => (
          ...
          {!cur ? <Empty icon={<StickyNote size={40} />} text={t('common.empty')} action={<button className="btn-primary" onClick={create}><Plus size={16} />{t('notes.newNote')}</button>} /> : (
            <>
              <div className="flex items-center gap-2 p-3 border-b border-surface-300/60 flex-wrap">
                ...
                <button className="btn-icon" title={cur.archived ? t('notes.unarchive') : t('notes.archive')} onClick={() => update(cur.id, { archived: !cur.archived })}>{cur.archived ? <ArchiveRestore size={16} /> : <Archive size={16} />}</button>
                <button className="btn-icon" title={t('notes.exportMd')} onClick={exportMd}><Download size={16} /></button>
                <button className="btn-icon hover:text-rose-500" onClick={() => remove(cur.id)}><Trash2 size={16} /></button>
              </div>
              <div className="flex items-center gap-3 px-3 py-2 border-b border-surface-300/60">
